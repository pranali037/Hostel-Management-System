...
#$password = password_hash($_POST['password'], PASSWORD_BCRYPT);
$query="insert into  registration(roomno,seater,feespm,foodstatus,stayfrom,duration, feesp, feest,course,regno,firstName,middleName,lastName,gender,contactno,emailid,password,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisiississssissississsisssi',$roomno,$seater,$feespm,$foodstatus,$stayfrom,$duration,$feesp,$feest,$course,$regno,$fname,$mname,$lname,$gender,$contactno,$emailid,$password,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode);
$stmt->execute();
...
<div class="col-sm-8">
<input type="text" name="feesp" id="feesp"  class="form-control" >
</div>
</div>
...
<div class="col-sm-8">
<select name="feest" class="form-control" required="required">
